...
def to_markdown(payload: dict) -> str:
    """Convert preprocessed payload to LLM-friendly markdown format."""
    parts = []
    ...
    for table in payload.get("tables", []):
        ...
        if rows:
            # Convert rows to markdown table: first row as header, separator, then data
...
def preprocess(path: Path, force: bool = False) -> Path:
    ...
    # Always write .json (required for schema routing)
    out_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    # Additionally write .md for document types (for agent reading convenience)
    # Documents: docx, pdf, pptx get markdown output
    # Data files: xlsx, csv get json-only output
    if type_label in {"docx", "pdf", "pptx"}:
        md_path = OUTPUT_DIR / f"{path.stem}.preprocessed.md"
        markdown_content = to_markdown(payload)
